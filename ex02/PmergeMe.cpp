...
PmergeMe::PmergeMe(std::vector<int>& v) : _cnt(0), _range(v.size()) , _type("std::vector"), _ov(v){
...
int	PmergeMe::_binaryInsert(const int& here, int end, std::vector<int>& mc){
	int start = 0;
	int mid;
	const int val = _ov[here];
	while (start < end){
